...
class ConditionalSiFall(nn.Module):

    def __init__(
            self, in_channels: int, latent_dim: int, hidden_dims: List = None, **kwargs
    ) -> None:
        super(ConditionalSiFall, self).__init__()
    ...
    def forward(self, input: Tensor, **kwargs) -> List[Tensor]:
        source, target = input[:, :3], input[:, 3:]
        mu, log_var, pooling_indices = self.encode(source)
        z = self.reparameterize(mu, log_var)
        reconstructed = self.decode(z, pooling_indices)
        return [reconstructed, target, mu, log_var]
