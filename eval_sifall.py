...
def save_thread(image_data, out_file, in_file):
    ...
    image_in = Image.open(in_file).convert('L')
    image_in = image_in.resize(size=(256, 256), resample=Image.LANCZOS)
    image_in = np.array(image_in, dtype=np.float32) / 255.0

    image_data = image_data.convert('L')
    image_data = image_data.resize(size=(256, 256), resample=Image.LANCZOS)
    image_data = np.array(image_data, dtype=np.float32) / 255.0

    mse_image = (image_in - image_data) ** 2
    out_file = out_file.replace('_rec.jpg', '.npy')
    np.save(out_file, mse_image)
...
batch_size = 32
exp_name = 'case_azi_fft.v9.1'
ckpt_path = glob.glob(f'logs/SiFall/{exp_name}/checkpoints/best*.ckpt')[0]
...
model = vae_models['SiFall'](name='SiFall', in_channels=3, latent_dim=128)
...
data = glob.glob(f'/playpen2/ttoha12/weapon/azi_fft/{exp_name}/*/*/*.jpg')
total_images = len(data)
assert total_images > 0, 'dataset is empty'
print(f'{exp_name}: found {total_images} files')

os.makedirs(out_dir, exist_ok=True)
input_images, in_paths, out_paths = [], [], []
pbar = tqdm(total=total_images)
for img_idx, file_path in enumerate(data):
    img_data = Image.open(file_path)
    img_data = transform(img_data).unsqueeze(dim=0).cuda()
    input_images.append(img_data)
    in_paths.append(file_path)

    out_path = os.path.basename(file_path)
    out_path = out_path.replace('.jpg', '_rec.jpg')
    out_paths.append(out_path)

    if img_idx % batch_size == 0 or img_idx == total_images - 1:
